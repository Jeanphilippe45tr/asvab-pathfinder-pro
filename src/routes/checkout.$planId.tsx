...
import { simulatePayment } from "@/lib/user.functions";
...
function Checkout() {
  ...
  async function handlePay() {
    ...
    setProcessing(true);
    try {
      await pay({ data: { planId } });
      toast.success("Payment simulated — awaiting admin approval");
      nav({ to: "/dashboard" });
    } catch (e: any) {
      ...
      <section className="mx-auto max-w-2xl px-4 py-16 md:px-8">
        ...
        <Card className="p-8 shadow-card">
          <div className="flex items-center justify-between">
            ...
            <div className="rounded-md bg-secondary p-3"><CreditCard className="h-5 w-5" /></div>
          </div>
          <div className="mt-6 rounded-md border-l-4 border-accent bg-accent/10 p-4 text-sm">
            <strong>Simulation mode.</strong> No real payment will be processed. This is for testing only.
          </div>
          <ul className="mt-6 space-y-2 text-sm text-muted-foreground">
            {(plan.features as string[]).map((f: string) => <li key={f}>• {f}</li>)}
          </ul>
          <Button size="lg" className="mt-8 w-full bg-gold text-gold-foreground hover:opacity-90" onClick={handlePay} disabled={processing}>
            {processing ? "Processing…" : session ? `Pay $${(plan.price_cents / 100).toFixed(2)} (simulated)` : "Sign in to continue"}
          </Button>
          <p className="mt-4 flex items-center justify-center gap-1 text-center text-xs text-muted-foreground">
            <ShieldCheck className="h-3 w-3" /> After payment, your subscription awaits admin approval.
          </p>
